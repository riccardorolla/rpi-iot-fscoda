...
Nel progetto utilizzerò \texttt{Raspberry\# IO}, è una libreria che consente di utilizzare le funzionalità GPIO di raspberry Pi su ambiente .NET/Mono, questa libreria è utilizzabile sia con il linguaggio \texttt{C\#} che con \texttt{F\#}.
...
\begin{itemize}
   	...
   	\item Accesso ai PIN GPIO attraverso 3 modalità:semplice (con uso di file) attraverso memoria, e  completa (memoria attraverso "pseudo-interrupt"). Di default  è usata questa ultima.
   	\item Indirizzamento con il numero Pin o con il numero di connettore pin
	\item Assegnamento dei Pin in base al modello e revisione del Raspberry Pi
	\item Uso controllato delle risorse utilizzando un componente IDisposable e la capacità di utilizzare il rilevamento delle soglie , invece di polling
	\item Supporto al di sotto del millisecondo per il polling dei pin di input
	
	\item Permette di personalizzare il nome dei Pin per consentire una maggiore leggibilità del codice
	\item Facilità di Uso, configurazione dichiarativa  dei Pin. Possibilità di ripristinare la polarità dei Pin (0/1). Permette di utilizzare un pin in input come uno switch button.  
	\item Genera un evento quando lo status di un pin cambia , utlizzando il polling
