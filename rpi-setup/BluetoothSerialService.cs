...
namespace RoverSetup
{
	...
	{
		// Debugging
		private const string TAG = "BluetoothSerialService";
		private const bool Debug = true;
	
		// Name for the SDP record when creating server socket
		private const string NAME = "RoverSetup";
		...
		private void SetState (int state)
		{
			...
			_handler.ObtainMessage (RoverBluetooth.MESSAGE_STATE_CHANGE, state, -1).SendToTarget ();
		}
		...
		public void Connected (BluetoothSocket socket, BluetoothDevice device)
		{
			...
			var msg = _handler.ObtainMessage (RoverBluetooth.MESSAGE_DEVICE_NAME);
			Bundle bundle = new Bundle ();
			bundle.PutString (RoverBluetooth.DEVICE_NAME, device.Name);
			msg.Data = bundle;
		...
		private void ConnectionFailed ()
		{
			...
			var msg = _handler.ObtainMessage (RoverBluetooth.MESSAGE_TOAST);
			Bundle bundle = new Bundle ();
			bundle.PutString (RoverBluetooth.TOAST, "Unable to connect device");
			msg.Data = bundle;
			_handler.SendMessage (msg);
		}
		...
		public void ConnectionLost ()
		{
			...
			var msg = _handler.ObtainMessage (RoverBluetooth.MESSAGE_TOAST);
			Bundle bundle = new Bundle ();
			bundle.PutString (RoverBluetooth.TOAST, "Device connection was lost");
			msg.Data = bundle;
			_handler.SendMessage (msg);
		}
		...
		private class AcceptThread : Thread
		{
			...
			private BluetoothSerialService _service;
		...
		private class ConnectedThread : Thread
		{
			...
			public override void Run ()
			{
				...
				while (true) {
					try {
						...
						_service._handler.ObtainMessage (RoverBluetooth.MESSAGE_READ, bytes, -1, buffer)
							.SendToTarget ();
					} catch (Java.IO.IOException e) {
			...
			{
				try {
					mmOutStream.WriteAsync (buffer, 0, buffer.Length);
	
					// Share the sent message back to the UI Activity
					_service._handler.ObtainMessage (RoverBluetooth.MESSAGE_WRITE, -1, -1, buffer)
						.SendToTarget ();
				} catch (Java.IO.IOException e) {
